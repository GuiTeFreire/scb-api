...
class HttpEquipamentoRepository(EquipamentoRepository):
    ...
    def trancar_tranca(self, id_tranca, id_bicicleta):
        url = f"{self.base_url}/tranca/{id_tranca}/trancar"
        response = httpx.post(url, json={"bicicleta": id_bicicleta})
        return response.status_code == 200

    def destrancar_tranca(self, id_tranca):
        response = httpx.post(f"{self.base_url}/tranca/{id_tranca}/destrancar")
        return response.status_code == 200
